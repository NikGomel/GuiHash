...
QString GuiHash::ReadFileByte(QString fileName)
{
    try {
        ...
        if (ui->actionbelt_hash256->isChecked())
        {
     ...
     QCoreApplication::processEvents();
        ...
        if(status)
        {
            ...
            QByteArray resoult = chFile->Resoult();
        ...
        else
        {
            ...
            int l = 128;
            if (ui->actionbash384->isChecked())
            {
                l = 192;
            }
            if (ui->actionbash512->isChecked())
            {
                l = 256;
            }
           ...
           QCoreApplication::processEvents();
            ...
            if(status)
            {
                ...
                QByteArray resoult = ch->BashResult();
                QString hash (resoult.toHex());
                hash = hash.toUpper();
            delete  ch;
            return hash;
    ...
    try {
       ...
       // Chash *ch = new Chash();
...

        {
          ...
          QByteArray resoult = chDir->Resoult();
          QString hash (resoult.toHex());
          hash = hash.toUpper();
          delete  chDir;
          return hash;
...
void GuiHash::on_actionbelt_hash256_triggered()
{
    ui->actionbash256->setChecked(false);
    ui->actionbash384->setChecked(false);
    ui->actionbash512->setChecked(false);
    ui->actionbelt_hash256->setChecked(true);
     ui->statusBar->showMessage("Выбран алгоритм belt-hash256");
}

void GuiHash::on_actionbash256_triggered()
{
    ui->actionbash256->setChecked(true);
    ui->actionbash384->setChecked(false);
    ui->actionbash512->setChecked(false);
    ui->actionbelt_hash256->setChecked(false);
       ui->statusBar->showMessage("Выбран алгоритм bash256");
}

void GuiHash::on_actionbash384_triggered()
{
    ui->actionbash256->setChecked(false);
    ui->actionbash384->setChecked(true);
    ui->actionbash512->setChecked(false);
    ui->actionbelt_hash256->setChecked(false);
     ui->statusBar->showMessage("Выбран алгоритм bash384");
}

void GuiHash::on_actionbash512_triggered()
{
    ui->actionbash256->setChecked(false);
    ui->actionbash384->setChecked(false);
    ui->actionbash512->setChecked(true);
    ui->actionbelt_hash256->setChecked(false);
     ui->statusBar->showMessage("Выбран алгоритм bash512");
}
void GuiHash::closeEvent(QCloseEvent *event)
{
